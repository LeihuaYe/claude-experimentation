...
A small, dependency-light toolkit for trustworthy experiment analysis, usable as Claude Code skills or plain Python libraries. Skills: `ab-design` (pre-launch sizing) and `ab-readout` (post-launch analysis).
...
- `tests/test_design.py`, `tests/test_readout.py` — validate the statistics against ground truth (design is checked against Monte-Carlo power).
...
## Roadmap
`ab-design` and `ab-readout` ship today. Planned: **sequential / always-valid** inference, **heterogeneous effects** (CATE / meta-learners), and **causal inference without randomization** (DiD / synthetic control).
